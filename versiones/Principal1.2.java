...
public class Principal {
	...
	private static void introducirRango(BigInteger bigRangoMinimo, BigInteger bigRangoMaximo, int numHilos){
		...
		//Datos datos[] = new Datos[numHilos];
        ...
        for (BigInteger i = new  BigInteger("0"); i.compareTo(BigInteger.valueOf(numHilos))<0 ; i = i.add(BigInteger.valueOf(1))){
            ...
            if (i.compareTo(BigInteger.valueOf(numHilos-1))==0){
                ...
                Datos datos;
                h[i.intValue()] = new Hilo(RangoMinimoProvisional, bigRangoMaximo, datos = new Datos());
            }else{
                ...
                Datos datos;
                h[i.intValue()] = new Hilo(RangoMinimoProvisional, RangoMaximoProvisional, datos = new Datos());
        ...
        System.out.println(numMaxHilos(h));
    }// introducirRango()
    ...
    private static BigInteger numMaxHilos(Hilo h[]) {
    	BigInteger numMaximo = new BigInteger("0");
    	for (int i = 0; i<h.length; ++i) {
    		if(numMaximo.compareTo(h[i].getDatosNumMaximo())<0) numMaximo = h[i].getDatosNumMaximo();
        }
    	return numMaximo;
    }
